	...
	var shop_name = "Starly Shoppe"; // NAMA TOKO ONLINE
	var domain = "http://starly-shoppe.com/mobile/"; // DOMAIN URL ADMIN
	var admin_url = domain;
	
	var base_url = domain+"_api_/android"; // URL API
	var base_url_media = admin_url+"media"; // DIREKTORI PENYIMPANAN IMAGE DI HOSTING
	var dir_image = "Pictures/Starly-shoppe"; // DIREKTORI PENYIMPANAN IMAGE DI SD CARD
	var token = "bbb90c3104df6868f44d591f2be11f61"; // ISI DENGAN TOKEN 
	...
	$( document ).on( "click", ".menu_page_myorder_ready", function() {
		...
		$.post(base_url+"/list_order_ready", {token: token, customer_id: customer_id},
		   function(data){
			...
			$("#listing_order_ready").html("");
			var order_item = data.order;
			
			for(var i=0;i < order_item.length;i++)
			{
				$("#listing_order_ready").append("<label>"+order_item[i].prod_name+" - "+order_item[i].variant+" - ("+order_item[i].qty+")</label>");
			}
		...
		window.location = "#page_myorder_ready";
	});	
	
	
	// MY ORDER PO
	$( document ).on( "click", ".menu_page_myorder_po", function() {
		...
		$.post(base_url+"/list_order_po", {token: token, customer_id: customer_id},
		   function(data){
			...
			$("#listing_order_po").html("");
			var order_item = data.order;
			
			for(var i=0;i < order_item.length;i++)
			{
				$("#listing_order_po").append("<label>"+order_item[i].prod_name+" - "+order_item[i].variant+" - ("+order_item[i].qty+")</label>");
			}
		...
		window.location = "#page_myorder_po";
	});	
						...
						$.post(base_url+"/get_ship_rates_cost",{token: token, kota_id: data_customer.kota_id},
							   function(data_cost){
								 ...
								 if(data_cost.status == 'Invalid Token')
								{
									...
									$("#total_dropship1_ongkir_edit").val(total_shipping_cost);
	...
	*/
	...
	$( document ).on( "click", ".btn_process_dropship1", function() {
		...
		var dropship_ongkir =  $("#dropship1_ongkir_text").val();
		var dropship_total_all = parseFloat(dropship_ongkir) + dropship_total;
		var order_tipe = $("#dropship1_tipe").val();
	...
	$( document ).on( "click", ".btn_process_dropship2", function() {
		...
		var dropship_ongkir =  $("#dropship2_ongkir_text").val();
		var dropship_total_all = parseFloat(dropship_ongkir) + dropship_total;
